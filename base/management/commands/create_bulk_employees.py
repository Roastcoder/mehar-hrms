"""
Django management command to create bulk employees
Usage: python manage.py create_bulk_employees
"""
...
class Command(BaseCommand):
    help = 'Create bulk employees from predefined list'
    ...
    def handle(self, *args, **options):
        ...
        for badge_id, name, phone, email in employees_data:
            ...
            if User.objects.filter(email=email).exists() or Employee.objects.filter(email=email).exists():
                self.stdout.write(f"SKIPPED: {name} - {email} (already exists)")
                skipped_count += 1
                continue
            ...
            try:
                with transaction.atomic():
                    ...
                    self.stdout.write(f"CREATED: {badge_id} - {name} - Password: {password}")
                    created_count += 1
        ...
        self.stdout.write(f"Total: {len(employees_data)}")
        self.stdout.write(self.style.SUCCESS('Employee creation completed!'))